...
### 🔑 Админ паролі қабылданбай жатса

> 📖 **Толық қадамдық нұсқаулық:** [docs/ADMIN-PASSWORD.md](docs/ADMIN-PASSWORD.md)

Пароль дерекқорда bcrypt hash түрінде сақталады. Ол **бірінші деплойда**
жасалады, ал дерекқор кейінгі деплойларда өмір сүре береді.

**Шешім:** Render → `kz-history-rpg` сервисі → **Environment** →
`ADMIN_PASSWORD` мәнін жаңасына ауыстырып, **Save changes** басыңыз.
Сервис қайта іске қосылғанда лог файлында мынау көрінеді:

```
[ADMIN] Пароль ADMIN_PASSWORD орта айнымалысынан жаңартылды.
```

Осыдан кейін жаңа пароль бірден жұмыс істейді.

| Жағдай | Не болады |
|---|---|
| `ADMIN_PASSWORD` өзгертілді | Hash автоматты жаңарады ✅ |
| `ADMIN_PASSWORD` өзгермеді | Ағымдағы пароль сақталады |
| Панель арқылы пароль қойылды, `ADMIN_PASSWORD` жоқ | Панельдегі пароль сақталады |

> ⚠️ Егер `ADMIN_PASSWORD` орта айнымалысы Render-де тұрса, ол әр рестартта
> басымдыққа ие. Паролді тек панель арқылы басқарғыңыз келсе, Render-ден
> `ADMIN_PASSWORD` жолын мүлдем өшіріп тастаңыз.
